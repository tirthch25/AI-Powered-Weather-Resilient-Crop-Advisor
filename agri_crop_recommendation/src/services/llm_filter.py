...
def _get_gemini_client():
    ...
    try:
        ...
        logger.info(
            f"Gemini filter client loaded (key #{_gemini_key_index % len(_GEMINI_KEYS) + 1} "
...
def llm_filter_crops(
    ...
    try:
        ...
        if provider == "gemini" and any(kw in err_str for kw in ("quota", "429", "rate")):
            logger.warning("Gemini rate limit hit in filter — rotating key")
            _rotate_gemini_key()
        logger.warning(
            f"LLM filter failed — fallback active ({provider}): {e}\n"
            + traceback.format_exc()
        )
        return None
